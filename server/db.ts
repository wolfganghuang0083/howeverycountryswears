...
export async function getCardRatings(countrySlug: string, cardNumbers: number[]) {
  ...
  for (const r of results) {
    map[r.cardNumber] = { avg: r.avgRating, count: r.count };
  }
  return map;
}
...
export async function getUserDashboardStats(userId: number) {
  const db = getDb();
  const [user] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
  const [{ submissionCount }] = await db.select({ submissionCount: sql<number>`COUNT(*)::int` }).from(submissions).where(eq(submissions.userId, userId));
  const [{ ratingCount }] = await db.select({ ratingCount: sql<number>`COUNT(*)::int` }).from(ratings).where(eq(ratings.userId, userId));
  return {
